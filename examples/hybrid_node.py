...
parser.add_argument("--attempt_load_state_dict", action="store_true", default=True)
...
parser.add_argument("--cache_dir", type=str, default=os.path.expanduser("/data/tyj/Rel-LLM-master/cache/baselines"), )
...
def self_supervised_loss(model, batch, tau=0.2):
    # 两个增强版本（比如 dropout、mask 特征）
    z1 = model(batch, task.entity_table)
    z2 = model(batch, task.entity_table)
...
def train() -> float:
    ...
    for batch in tqdm(loader_dict["train"], total=total_steps):
        ...
        # ✅ self-supervised 版本
        loss = self_supervised_loss(model, batch)
...
@torch.no_grad()
def test(loader: NeighborLoader, add_noise: bool = False, sigma_ratio: float = 0.1) -> np.ndarray:
    model.eval()

    pred_list = []
    for batch in loader:
        batch = batch.to(device)
        pred = model(batch, task.entity_table, add_noise=add_noise, sigma_ratio=sigma_ratio)
...
# Model training
# =====================
model = Model(data=data, col_stats_dict=col_stats_dict, num_layers=args.num_layers, channels=args.channels, out_channels=out_channels, aggr=args.aggr, model_type=args.model_type,norm="batch_norm", ).to(
    device)

STATE_DICT_PTH = "/data/tyj/Rel-LLM-master/cache/checkpoints/GNN/{args.dataset}_{args.task}_state_dict.pth"
...
if args.testing:
    if os.path.exists(STATE_DICT_PTH):
        print(f"🔹 加载源域权重文件：{STATE_DICT_PTH}")
        state_dict = torch.load(STATE_DICT_PTH, map_location="cpu")  # 或 map_location=device
    else:
        raise FileNotFoundError(f"❌ 未找到源域权重文件：{STATE_DICT_PTH}")
    ...
    test_pred1 = test(loader_dict["test"])
    test_metrics1 = task.evaluate(test_pred1)
    print(f"Best test metrics: {test_metrics}")
    print(f"Best drift test metrics: {test_metrics1}")
