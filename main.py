...
if __name__ == '__main__':
    ...
    def is_integer_column(col: torch.Tensor, tol: float = 1e-6) -> bool:
        """
        判断一列数值是否为“整数型”（允许 float dtype，但值为整数）
        """
    ...
    """def get_row_level_prob_decay(epoch, num_epochs, start=0.7, end=0.3, mode="linear"):
        
        if mode == "linear":
            # 线性衰减
            p = start - (start - end) * (epoch / num_epochs)
        elif mode == "sigmoid":
            # Sigmoid 衰减：前期下降慢，中期下降快，末期趋于 end
            progress = epoch / num_epochs
            factor = 1 / (1 + math.exp(10 * (progress - 0.5)))  # 注意符号取反
    ...
    def get_row_level_prob_decay(epoch, num_epochs, start=0.7, end=0.3, mode="cosine"):
        """
        根据 epoch 动态衰减行级任务比例
        start: 起始行级比例（epoch=0）
        end: 最终行级比例（epoch=num_epochs）
        """
        progress = epoch / num_epochs
        ...
        if mode == "linear":
            ...
            # Cosine 衰减：前期下降慢，中期较快，末期平滑收敛到 end
    ...
    """for batch in loader_dict_tgt["train"]:
        print(batch.keys())  # 应该有 'y' 或者对应列"""
