...
class LinearEncoder(StypeEncoder):
    ...
    def encode_forward(
        ...
        feat: Tensor,
        col_names: list[str] | None = None,
        #add_noise=False, sigma_ratio=0.1
    ) -> Tensor:

        """if add_noise:
            print("加噪")
            std_col = torch.std(feat, dim=0, keepdim=True)
            noise = torch.randn_like(feat) * (sigma_ratio * std_col)
            feat = feat + noise"""
...
class LinearEmbeddingEncoder(StypeEncoder):
    ...
    def encode_forward(
        ...
        for idx, col_dim in enumerate(self.emb_dim_list):
            ...
            x_col = feat.values[:, start_idx:end_idx]

            """if add_noise:
                # 按列 embedding 的 std 生成噪声
                std_col = torch.std(x_col, dim=0, keepdim=True) + 1e-6
                noise = torch.randn_like(x_col) * (sigma_ratio * std_col)
                x_col = x_col + noise"""

            x_lin = x_col @ self.weight_list[idx]
            x_lins.append(x_lin)
            start_idx = end_idx
